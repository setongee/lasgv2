...
export default function Mdas() {
 ...
 const [queryResults, setQueryResults] = useState(mdaDB);
 ...
 useEffect(() => {
  ...
  const fuse = new Fuse(mdaDB, fuseOptions);
  const results = fuse.search(query);
  const queriedRes =  query ? results.map(res => res.item) : mdaDB;
...
if (indexFilter !== '') {
  ...
  const filterByIndex = mdaDB.filter(e=>e.index === indexFilter);
  setQueryResults(filterByIndex);

} else {

  setQueryResults(mdaDB);
